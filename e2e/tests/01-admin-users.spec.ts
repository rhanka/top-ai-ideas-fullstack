...
const DISABLE_BUTTON_RE = /^(Disable|Désactiver)$/i;
const REACTIVATE_BUTTON_RE = /^(Reactivate|Réactiver)$/i;
...
test.describe('Admin · Utilisateurs (Paramètres)', () => {
  ...
  test('sécurité: l’admin ne peut pas se désactiver / supprimer (UI)', async ({ page }) => {
    ...
    await expect(adminRow.locator('button').filter({ hasText: DISABLE_BUTTON_RE })).toHaveCount(0);
    await expect(adminRow.locator('button').filter({ hasText: DELETE_BUTTON_RE })).toHaveCount(0);
  });
  ...
  test('l’admin peut disable/reactivate/delete un utilisateur non-admin (flux UI)', async ({ page }) => {
    ...
    await victimRow().locator('button').filter({ hasText: DISABLE_BUTTON_RE }).click();
    await expect(victimRow()).toContainText(/disabled_by_admin|désactivé par admin|disabled by admin/i, { timeout: 15_000 });

    // Reactivate
    await victimRow().locator('button').filter({ hasText: REACTIVATE_BUTTON_RE }).click();
    await expect(victimRow()).toContainText(/active|actif/i, { timeout: 15_000 });

    // Disable then Delete (définitif)
    await victimRow().locator('button').filter({ hasText: DISABLE_BUTTON_RE }).click();
    await expect(victimRow()).toContainText(/disabled_by_admin|désactivé par admin|disabled by admin/i, { timeout: 15_000 });

    await victimRow().locator('button').filter({ hasText: DELETE_BUTTON_RE }).click();
