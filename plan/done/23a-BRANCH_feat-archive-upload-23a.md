...
- [x] `attention` BR23a-EX2 — Touch `Makefile` only to make `db-seed-test` prefer the built `dist/tests/utils/seed-test-data.js` artifact when present, with a `tsx tests/utils/seed-test-data.ts` fallback for dev-mounted API containers. Reason: PR #123 CI E2E/VSCode jobs on 2026-04-26 failed before any test ran because the branch's `tsx`-only command expected `/app/tests/utils/seed-test-data.ts` inside the production API image, but that image ships only `dist`. Impact: E2E seeding only; no product/runtime behavior change. Rollback: collapse back to a single path once the test harness standardizes on either the built artifact or mounted source script.
- [x] `clarification` BR23a-D2 — Root UAT reported `File too large` on archive upload. The API ceiling was still `25 MiB`; this branch now raises the document upload cap to `100 MiB` and covers it with an API regression test (`26 MiB` upload accepted).
- [x] `clarification` BR23a-D3 — Root UAT exposed a legacy-status bug: archives already flagged as `indexingSkipped` could still render as `failed` in the documents list. The API now heals download-only archive rows to effective `ready` status on list/read, so non-indexable files no longer show a false error badge.
- [ ] `investigate` BR23a-B1 — `make test-api-endpoints API_PORT=9081 UI_PORT=5281 MAILDEV_UI_PORT=1181 ENV=test-feat-archive-upload-23a` remains unstable after the rebase on suites outside archive scope. The first full run failed in `tests/api/chat.test.ts` (`handles concurrent steering submissions without HTTP 500`), but the scoped rerun of that case and the full `tests/api/chat.test.ts` file both passed; the second full run failed instead in `tests/api/docx.test.ts` (`processes publishing jobs even when AI queue slots are saturated`, 60000 ms timeout) and `tests/api/chat-tools.test.ts` (`returns a proposal for comment_assistant suggest`, missing `tool_call_result`). Archive-focused validations are green (`tests/api/documents.test.ts`, `tests/utils/documents.test.ts`, `tests/04-documents-ui-actions.spec.ts`, `tests/vscode/01-vscode-chat-streaming.spec.ts`). Resolve root cause or confirm CI outcome before closure.
...
- [ ] **Lot 2 — Final validation**
  ...
  - [x] `make typecheck-ui API_PORT=9081 UI_PORT=5281 MAILDEV_UI_PORT=1181 ENV=test-feat-archive-upload-23a`
  - [x] `make lint-ui API_PORT=9081 UI_PORT=5281 MAILDEV_UI_PORT=1181 ENV=test-feat-archive-upload-23a`
  - [x] `make test-api-unit API_PORT=9081 UI_PORT=5281 MAILDEV_UI_PORT=1181 ENV=test-feat-archive-upload-23a`
  - [ ] `make test-api-endpoints API_PORT=9081 UI_PORT=5281 MAILDEV_UI_PORT=1181 ENV=test-feat-archive-upload-23a`
  - [x] `make test-ui SCOPE=tests/utils/documents.test.ts API_PORT=9081 UI_PORT=5281 MAILDEV_UI_PORT=1181 ENV=test-feat-archive-upload-23a`
  - [x] `make test-e2e E2E_SPEC=tests/04-documents-ui-actions.spec.ts API_PORT=9083 UI_PORT=5283 MAILDEV_UI_PORT=1183 ENV=e2e-feat-archive-upload-23a`
  - [x] `make test-e2e-vscode E2E_SPEC=tests/vscode/01-vscode-chat-streaming.spec.ts OPENVSCODE_PORT=3115 API_PORT=9083 UI_PORT=5283 MAILDEV_UI_PORT=1183 ENV=e2e-feat-archive-upload-23a`
