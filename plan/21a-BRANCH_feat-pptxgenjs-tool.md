...
## Objective
Add one more chat tool call for PowerPoint generation, backed by `pptxgenjs`, with generated PPTX files handled in chat alongside generated DOCX files. The branch owns the generic presentation tool and generated-file chat handling. It does not own profile exports.
...
- Product principle accepted; Q&A must complete before writing the real spec.
...
- [ ] `attention` BR21a-Q1-Q8 are tracked in `tmp/feat-pptxgenjs-tool-21a/plan/21a-BRAINSTORM_pptxgenjs-tool.md` and must be validated before spec.
...
- [ ] **Lot 1 — Brainstorming and Q&A**
  ...
  - [ ] Create `spec/SPEC_EVOL_PPTXGENJS_TOOL.md` from validated answers only.
