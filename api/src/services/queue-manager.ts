...
export class QueueManager {
  ...
  private formatAutoGenerationFieldComment(sectionKey: string): string {
    return `Field "${sectionKey}" was generated by AI assistant. Please review and adjust if needed.`;
  }
  ...
  private async createAutoGenerationFieldComments(opts: {
    ...
    for (const sectionKey of uniqueSectionKeys) {
      ...
      await db.insert(comments).values({
        ...
        status: 'open',
        threadId: createId(),
        content: this.formatAutoGenerationFieldComment(sectionKey),
        toolCallId: null,
        createdAt: now,
        updatedAt: now
      });
  ...
  private async processOrganizationEnrich(data: OrganizationEnrichJobData, jobId: string, signal?: AbortSignal): Promise<void> {
    const { organizationId, organizationName, model, initiatedByUserId } = data;
  ...
  private async processMatrixGenerate(data: MatrixGenerateJobData, signal?: AbortSignal): Promise<void> {
    const { folderId, organizationId, model, initiatedByUserId } = data;
  ...
  private async processUseCaseList(data: UseCaseListJobData, signal?: AbortSignal): Promise<void> {
    const { folderId, input, organizationId, matrixMode, model, useCaseCount, initiatedByUserId } = data;
  ...
  private async processUseCaseDetail(data: UseCaseDetailJobData, signal?: AbortSignal): Promise<void> {
    const { useCaseId, useCaseName, folderId, matrixMode, model, initiatedByUserId } = data;
  ...
  private async processExecutiveSummary(data: ExecutiveSummaryJobData, signal?: AbortSignal): Promise<void> {
    const { folderId, valueThreshold, complexityThreshold, model, initiatedByUserId } = data;
