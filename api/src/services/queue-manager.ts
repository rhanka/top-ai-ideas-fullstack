...
export class QueueManager {
  ...
  private formatAutoGenerationFieldComment(sectionKey: string, locale: AppLocale): string {
    if (locale === 'en') {
      return `Field "${sectionKey}" was generated by AI assistant. Please review and adjust if needed.`;
    }
    return `Le champ "${sectionKey}" a ete genere par l'assistant IA. Merci de le verifier et de l'ajuster si necessaire.`;
  }
  ...
  private async createAutoGenerationFieldComments(opts: {
    ...
    for (const sectionKey of uniqueSectionKeys) {
      ...
      await db.insert(comments).values({
        ...
        status: 'open',
        threadId: createId(),
        content: this.formatAutoGenerationFieldComment(sectionKey, locale),
        toolCallId: null,
        createdAt: now,
        updatedAt: now
      });
