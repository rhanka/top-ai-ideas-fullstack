...
- If BR-14c cannot consume `@entropic/llm-mesh` directly from `api/` under the BR-14f workspace wiring, the branch is incomplete. A relative source import such as `../../packages/...` is not a valid exit criterion.
...
- CI/CD detects `packages/llm-mesh/**`, validates typecheck/test/build/pack on PRs, and publishes `@entropic/llm-mesh` on `main` through npm Trusted Publishing with GitHub OIDC, without a long-lived npm publish token.
...
Goal: modularize reusable chat-service behavior above `@entropic/llm-mesh` after BR-14c has completed model runtime cutover.
...
- Use `@entropic/llm-mesh` as the only model-access layer.
