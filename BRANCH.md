...
- [x] `validation` BR14f-V7 — Isolated dev-stack capacity proof on 2026-05-06: `make dev API_PORT=8715 UI_PORT=5115 MAILDEV_UI_PORT=1015 ENV=chore-node-workspace-monorepo-14f` boots the branch workspace stack; `make ps API_PORT=8715 UI_PORT=5115 MAILDEV_UI_PORT=1015 ENV=chore-node-workspace-monorepo-14f` shows API, Postgres, and Maildev healthy, UI up on `http://localhost:5115`, and API exposed on `http://localhost:8715`. This is not user UAT. User smoke UAT remains on root `ENV=dev` with the user's data.
...
- [x] `attention` BR14f-EX2 — `README.md` and `README.fr.md` join BR14f scope for durable target-architecture documentation. Rationale: the activation contract must survive after `plan/14f-BRANCH_chore-node-workspace-monorepo.md` moves to `plan/done/`. Impact: documentation only. Rollback: revert the README target-architecture paragraph.
- [x] `attention` BR14f-EX3 — `rules/architecture.md` joins BR14f scope for durable package-boundary rules. Rationale: future BR14c/14b/14a work needs a persistent rule that app roots stay at `api/` and `ui/`, reusable Node libraries live under `packages/*`, and package consumption must be proven through the root workspace. Impact: rules documentation only. Rollback: revert the architecture rule section.
