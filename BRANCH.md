...
- [ ] `investigate` BR23a-B1 — `make test-api-endpoints API_PORT=9081 UI_PORT=5281 MAILDEV_UI_PORT=1181 ENV=test-feat-archive-upload-23a` still fails after the rebase on suites outside archive scope (`tests/api/auth/*.test.ts`, `tests/api/chat.test.ts`). Scoped archive validation is green (`tests/api/documents.test.ts`, `tests/utils/documents.test.ts`, `tests/04-documents-ui-actions.spec.ts`). Resolve root cause or confirm CI outcome before closure.
