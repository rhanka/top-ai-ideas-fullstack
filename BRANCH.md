...
- [x] `resolved` BR14c-B6 — Root Playwright pre-UAT exposed Gemini 3 visible thought leakage in assistant content for a simple chat smoke. Fix: `buildGeminiRequestBody` now explicitly requests Gemini 3 low/high thinking levels with `includeThoughts: false`, keeps legacy thinking-budget paths from exposing thought summaries, and omits assistant history entries that already contain leaked Gemini internal thought markers before sending them back to Gemini. Validation: targeted Gemini request-body test, `llm-runtime-stream` test, `typecheck-api`, `lint-api`, raw Gemini API comparison, and root Playwright smoke.
