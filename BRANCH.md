...
- [x] `resolved` BR14c-B6 — Root Playwright pre-UAT exposed Gemini 3 visible thought leakage in assistant content for a simple chat smoke. Fix: `buildGeminiRequestBody` now explicitly requests Gemini 3 low/high thinking levels with `includeThoughts: false`, keeps legacy thinking-budget paths from exposing thought summaries, and unit coverage asserts Gemini thought parts stay out of `content_delta`. Validation: targeted Gemini request-body test, `llm-runtime-stream` test, `typecheck-api`, and `lint-api`.
