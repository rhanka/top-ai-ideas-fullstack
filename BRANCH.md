...
- [ ] Step 2 (commit 2, api wire + delete inline): replace the inline gate block at `chat-service.ts` lines 3534-3636 with `const gate = await this.runtime.applyContextBudgetGate({...}); if (gate.shouldContinue) { /* push deferredAccumulator + continue */ } else { /* preToolBudget = gate.preToolBudget; streamSeq = gate.streamSeq; contextBudgetReplanAttempts = gate.contextBudgetReplanAttempts; */ }`. Behavior preservation STRICT. Regression: typecheck-api + lint-api + test-pkg-chat-core + chat.test.ts (28/28) + chat-tools.test.ts (6/6) + chat-service-tools.test.ts (14/14).
