  ...
  - [x] Bug backlog (Lot 6)
    ...
    - [x] BUG-L6-29 — Web app / Chrome dark theme misses the same custom dark surfaces already handled for VSCode hosts.
      ...
      - [x] Validate the web dark-theme stylesheet with:
        - [x] `make typecheck-ui API_PORT=8705 UI_PORT=5105 MAILDEV_UI_PORT=1005 REGISTRY=local ENV=test-feat-vscode-plugin-v1`
    - [x] BUG-L6-30 — One assistant run that alternates visible answer chunks and reasoning/tool phases is still rendered as one assistant bubble instead of a linear multi-step timeline.
      ...
      - [x] Keep session preview based on the last visible message only (never reasoning/tool content).
    ...
    - [x] FEAT-L6-32 — Provide a safe local Playwright navigation/debug harness on top of the real `ENV=dev` stack.
      - [x] Consolidate dev helper services into `docker-compose.dev.yml` only:
        ...
        - [x] keep `make dev` unchanged (no helper profile auto-start),
        - [x] keep CI / test / e2e overlays unchanged.
      - [x] Refactor `Makefile` dev helper targets:
        - [x] preserve `up-dev-vscode`, `down-dev-vscode`, `ps-dev-vscode`, `logs-dev-vscode`,
        - [x] add `up-dev-playwright`, `down-dev-playwright`, `ps-dev-playwright`, `logs-dev-playwright`, `shell-dev-playwright`,
        - [x] add `test-e2e-dev E2E_SPEC=...` with no seed/reset/global-setup side effects.
      - [x] Add dedicated Playwright dev config/spec lane:
