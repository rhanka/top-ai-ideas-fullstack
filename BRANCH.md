...
- [x] `validation` BR16a-U2 — Formal Lot 5 root webapp UAT is now completed. Evidence:
...
- [x] `validation` BR16a-U4 — Live OAuth/Picker root UAT is closed. The target runtime now has working Google Drive credentials, a connected real account (`fabien.antoine@gmail.com`), successful Picker config generation, and a browser-proven return redirect to the root UI host.
