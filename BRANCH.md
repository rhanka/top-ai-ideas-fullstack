...
## Lot 22b-1 — Extract ChatRuntimeCheckpoint sub-class
- [x] Step 1 — Create `packages/chat-core/src/runtime-checkpoint.ts` (NEW, 235 LOC) with `ChatRuntimeCheckpoint` class holding VERBATIM bodies of `createCheckpoint` + `listCheckpoints` + `restoreCheckpoint`. Constructor receives `ChatRuntimeDeps` reference. Imports helpers `encodeChatCheckpointKey` + `summaryFromSnapshot` + `snapshotMessageFromRow` from `runtime.js` (those 3 helpers are now `export const` in runtime.ts to make them importable from the sub-class file; `parseChatCheckpointKey` stays module-private with its `void` lint stub). Gate: `make typecheck-pkg-chat-core` PASS (via `make test-pkg-chat-core` which runs the full type-aware Vitest pipeline → 237/237 tests still green, including 6 checkpoint tests still routed through `ChatRuntime`). Gate: `make typecheck-api` PASS (no API-side breakage from the new internal exports).
- [x] Step 2 — Wire `ChatRuntime` façade. Add `private readonly checkpoint: ChatRuntimeCheckpoint` field; constructor body now reads `this.checkpoint = new ChatRuntimeCheckpoint(deps)` (same `deps` reference passed by-reference, no state duplication). Replace the 3 method bodies (`createCheckpoint` 67 LOC body, `listCheckpoints` 16 LOC body, `restoreCheckpoint` 34 LOC body — 117 LOC total) with one-line delegators (`return this.checkpoint.<method>(opts)`). Public method signatures byte-identical (verified). Drop the verbose JSDoc trios above each method (the full per-method JSDoc is preserved on the sub-class methods in `runtime-checkpoint.ts`); keep a short delegation JSDoc above each façade wrapper. `runtime.ts`: 4983 → 4841 LOC (−142 net). Step 2 diff: 28+/170−. Gate: `make test-pkg-chat-core` PASS → 237/237 tests including the 6 `runtime-checkpoint.test.ts` cases now routed through the façade → sub-class. Gate: `make typecheck-api` PASS.
- [x] Step 3 — Full regression. Gates run (all green): `make typecheck-api` PASS, `make lint-api` PASS (0 errors, only pre-existing warnings), `make test-pkg-chat-core` 237/237 PASS (including 6 checkpoint tests via façade), `make test-api-endpoints SCOPE=tests/api/chat.test.ts` 28/28 PASS, `make test-api-endpoints SCOPE=tests/api/chat-tools.test.ts` 6/6 PASS, `make test-api-unit SCOPE=tests/unit/chat-service-tools.test.ts` 14/14 PASS. Total Lot 22b-1 deltas: `runtime.ts` 4983 → 4841 (−142 LOC, body removed −178 + delegators/field/import +36); `runtime-checkpoint.ts` NEW (235 LOC, 195 of which are the 3 method bodies + JSDoc); net repo +97 LOC (close to the +35 plan estimate, the extra comes from the more thorough JSDoc on the new sub-class file). `make down ENV=test-refacto-chat-service-core` clean. **Façade pattern validated** — Lot 22b-2 ChatRuntimeSession can reuse the same shape (constructor takes `deps`, class lives in `packages/chat-core/src/runtime-session.ts`, `ChatRuntime` adds a `private readonly session: ChatRuntimeSession` field + 4 one-line delegators).

## Lot 22b-2 — Extract ChatRuntimeSession sub-class
- [x] Step 1 — Create `packages/chat-core/src/runtime-session.ts` (NEW, 301 LOC) with `ChatRuntimeSession` class holding VERBATIM bodies of `listMessages` + `getSessionBootstrap` + `getSessionHistory` + `getMessageRuntimeDetails`. Constructor takes `(deps: ChatRuntimeDeps, checkpoint: ChatRuntimeCheckpoint)` — the sibling `ChatRuntimeCheckpoint` instance is required because `getSessionBootstrap` and `getSessionHistory` both call `this.listCheckpoints` (Lot 22b-0 Section D inventory missed this cross-sub-class dependency — Section B claim "no cross-call" was incorrect; passing the sibling by reference resolves it without duplicating the checkpoint body or going through the façade). All `this.listMessages` calls inside the new sub-class stay in-class (Session owns `listMessages`). Imports types from `runtime.js` and history projection helpers (`buildAssistantMessageHistoryDetails` + `buildChatHistoryTimeline` + `compactChatHistoryTimelineForSummary` + 2 history types) from `history.js`. Gate: `make typecheck-api` PASS. Gate: `make test-pkg-chat-core` 237/237 PASS (new file is orphan at this step — sub-class not yet wired in `ChatRuntime`; existing 17 session+message tests still cover the bodies in `runtime.ts`).
- [x] Step 2 — Wire `ChatRuntime` façade. Add `private readonly session: ChatRuntimeSession` field; constructor body now reads `this.session = new ChatRuntimeSession(deps, this.checkpoint)` (the `checkpoint` sub-class is constructed FIRST so it is available for the Session constructor). Replace the 4 method bodies (`listMessages` 22 LOC body, `getSessionBootstrap` 35 LOC body, `getSessionHistory` 47 LOC body, `getMessageRuntimeDetails` 46 LOC body — 150 LOC total) with one-line delegators (`return this.session.<method>(args)`). Drop the verbose JSDoc trios above each method (the full per-method JSDoc is preserved on the sub-class methods in `runtime-session.ts`); keep a short delegation JSDoc above each façade wrapper. Also collapse the now-unused `history.js` imports — only `ChatHistoryTimelineItem` remains needed (for the 2 result-type definitions L551/L565); the 5 other symbols (`buildAssistantMessageHistoryDetails` + `buildChatHistoryTimeline` + `compactChatHistoryTimelineForSummary` + `ChatHistoryMessage` + `ChatHistoryStreamEvent`) move with their callers into `runtime-session.ts`. Public method signatures byte-identical (verified — 4 method signatures match the pre-extraction shape exactly). `runtime.ts`: 4841 → 4655 LOC (−186 net). Step 2 diff: 31+/217−. Gate: `make typecheck-api` PASS. Gate: `make test-pkg-chat-core` 237/237 PASS (all 17 session+message tests now route through the façade → sub-class; `runtime-session.ts` coverage jumps from 0% to 96.42%).
- [x] Step 3 — Full regression. Gates run (all green): `make typecheck-api` PASS, `make lint-api` PASS (0 errors, only 184 pre-existing warnings — same baseline as post-Lot 22b-1), `make test-pkg-chat-core` 237/237 PASS (including 8 `runtime-session.test.ts` + 9 `runtime-message.test.ts` cases via façade), `make test-api-endpoints SCOPE=tests/api/chat.test.ts` 28/28 PASS, `make test-api-endpoints SCOPE=tests/api/chat-tools.test.ts` 6/6 PASS, `make test-api-unit SCOPE=tests/unit/chat-service-tools.test.ts` 14/14 PASS. Total Lot 22b-2 deltas: `runtime.ts` 4841 → 4655 (−186 LOC, body removed −217 + delegators/field/import +31); `runtime-session.ts` NEW (301 LOC, ~150 of which are the 4 method bodies + the rest is sub-class file header JSDoc + per-method JSDoc preserved from the pre-extraction trios + imports). Net repo +115 LOC (close to the +43 plan estimate; the extra comes from the more thorough JSDoc on the new sub-class file, mirroring the Lot 22b-1 pattern). `make down ENV=test-refacto-chat-service-core` clean. **Cross-sub-class dependency resolved** — Section D inventory was incomplete (claimed Session had no cross-class calls); the actual `this.listCheckpoints` cross-call from `getSessionBootstrap` / `getSessionHistory` is now routed through the sibling `ChatRuntimeCheckpoint` instance passed at construction time. This pattern (sibling injection) will be reusable for Lot 22b-3+ if other cross-sub-class dependencies surface during extraction.
