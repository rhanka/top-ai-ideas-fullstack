...
- [ ] **Lot 4B Chat session documents (upload + résumé + doc tool)**:
    ...
    - [!] UAT lot 4B
        ...
        - [!] Test: doc tool utilise les docs attachés à la session. (ne fonctionne pas)
        - [!] Test: accès interdit si session hors workspace (non testé)
- [ ] **Lot 4Bbis Prompt + session UX cleanup**:
    - [ ] Centralize chat system prompt in `default_prompts` (single base + context block injection).
    - [ ] Add explicit document availability block (session + focus-derived contexts).
    - [ ] Add mini prompt for conversation automation.
    - [ ] Replace hardcoded session titles tied to context with titles by AI (model: gpt-4.1-nano).
    - [ ] Add a dedicated default prompt for AI session titles.
    - [ ] `make typecheck` + `make lint`
    - [ ] UAT lot 4Bbis
        - [ ] Test: session title is generated by AI (no raw context ids).
        - [ ] Test: prompt uses session docs + focus context by default.
        - [ ] Test: historical context can be referenced when needed.
        - [ ] Test: doc tool utilise les docs attachés à la session. (ne fonctionne pas)
