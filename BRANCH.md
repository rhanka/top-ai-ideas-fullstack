...
- [x] `resolved` BR14c-B2 — Package publication lane is wired: `@entropic/llm-mesh` starts at `0.1.0`, package metadata includes repository/license/files/publish config, make-backed `build-llm-mesh`, `pack-llm-mesh`, and `publish-llm-mesh` targets exist, and `.github/workflows/ci.yml` adds PR validation plus `main`-only npm Trusted Publishing through GitHub OIDC (`id-token: write`) without a long-lived npm publish token.
...
- [ ] **Lot 6 — npm publication lane**
  ...
  - [x] Add make-backed npm publish target using GitHub OIDC Trusted Publishing only; no local secret files, no long-lived npm publish token, and no manual host publish.
  ...
  - [x] Add `main`-only CI publication after branch CI gates pass, using npm Trusted Publishing/OIDC with automatic provenance when supported by the CI environment.
  ...
  - [ ] Lot gate:
    ...
    - [x] Trusted Publishing readiness rerun after removing `NPM_TOKEN`: `make typecheck-llm-mesh API_PORT=8714 UI_PORT=5114 MAILDEV_UI_PORT=1014 ENV=test-feat-llm-mesh-sdk` and `make pack-llm-mesh API_PORT=8714 UI_PORT=5114 MAILDEV_UI_PORT=1014 ENV=test-feat-llm-mesh-sdk` passed with `node:24-bookworm-slim` / npm `11.12.1`.
